% untuk interval 60 menit (hari cerah)

Generationdata = readtable('D:\Generation data (2).xlsx');
Irradiationdata = readtable('D:\Irradiation data (2).xlsx');

TrainIp = table2array(Irradiationdata(25:4:2521,14:-2:8));     % Membaca data iradiasi dari file
TestIp = table2array(Generationdata(4:4:2500,14:-2:8));        % Membaca data generasi dari file
...
% Menghapus noise dari data (nilai lebih dari 50 atau kurang dari 0)
TrainIp(TestIp > 50) = [];
TestIp(TestIp > 50) = [];
TrainIp(TestIp <= 0) = [];
TestIp(TestIp <= 0) = [];
...
xlabel("Waktu (interval 60 menit)")
title("Unit Generation")

% Data untuk hari cerah 1 April
YTrainIp = table2array(Irradiationdata(25:4:121,16));        % Data input uji
YTestIp = table2array(Generationdata(4:4:100,16));           % Data target uji
...
% Definisi lapisan GRU
layers = [ ...
    sequenceInputLayer(numFeatures)
    gruLayer(numHiddenUnits)         % Menggunakan GRU, bukan LSTM
...
[net, YPred] = predictAndUpdateState(net, [XTrainIp(end-1); XTestIp(end-1)]);

% Melakukan prediksi dengan update jaringan
...
% Plot perbandingan prediksi dan observasi
figure
subplot(2,1,1)
plot(YTest)
hold on
